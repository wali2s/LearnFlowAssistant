...
private extension HomeView {
    ...
    var recentGoalsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Last goals")
                .font(.headline)
    ...
    var quickActionsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Button {
                ...
                Label(
                    !isSessionRunning ? "Start a new session" : "Resume current session",
                    systemImage: isSessionRunning ? "pause.circle.fill" : "play.circle.fill"
                )
                .font(.headline)
                .frame(maxWidth: .infinity)
                .foregroundStyle(.white)
    ...
    var dailyChallengeSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack {
                    Text("Today: \(viewModel.todayStudyMinutes) / \(viewModel.dailyStudyGoalMinutes) min")
                        .font(.caption)
                        .foregroundStyle(.secondary)
