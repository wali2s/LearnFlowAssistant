...
final class AppViewModel: ObservableObject {
    ...
    func deleteGoal(id: UUID) {
        ...
        if currentGoalTitle.isEmpty == false,
           goals.first(where: { $0.id == id }) == nil,
    ...
    private func sanitizeSelectedGoal() {
        ...
        if goals.first(where: { $0.id == selectedGoalId }) == nil {
            self.selectedGoalId = nil
        }
    }
...
extension AppViewModel {
    ...
    var totalStudyTimeText: String {
        formattedDuration(totalStudySeconds)
    }
    ...
    var goalState: [GoalState] {
        goals.map { goal in
            let matchingSessions = sessions.filter { $0.goalId == goal.id }
            let totalSeconds = matchingSessions.reduce(0) { $0 + $1.durationInSeconds }
    ...
    var filteredGoals: [LearningGoal] {
        ...
        let searchedGoals: [LearningGoal]

        if trimmedSearchText.isEmpty {
            searchedGoals = statusFilteredGoals
        } else {
            searchedGoals = statusFilteredGoals.filter { goal in
    ...
    var groupedRecentSessions: [SessionSection] {
        ...
        return grouped
            .map { date, sessions in
                SessionSection(date: date, sessions: sessions.sorted { $0.startedAt > $1.startedAt })
            }
    ...
    var chartGoalStats: [GoalState] {
        goalState.filter { $0.totalSeconds > 0 }
    }
    ...
    var averageSessionDurationText: String {
        ...
        let averageSeconds = totalStudySeconds / sessions.count
        return formattedDuration(averageSeconds)
    }
    ...
    var dailyGoalProgress: Double {
        ...
        case .studyMinutes:
            guard dailyStudyGoalMinutes > 0 else { return 0 }
            return min(Double(todayStudyMinutes) / Double(dailyStudyGoalMinutes), 1.0)

        case .completeSession:
            guard dailySessionGoalCount > 0 else { return 0 }
            return min(Double(todaySessionCount) / Double(dailySessionGoalCount), 1.0)
        }
    }
    ...
    var hasCompletedDailyGoal: Bool {
        switch dailyChallengeType {
        case .studyMinutes:
            return todayStudyMinutes >= dailyStudyGoalMinutes
        case .completeSession:
            return todaySessionCount >= dailySessionGoalCount
        }
    }
    ...
    var dailyChallengeText: String {
        ...
        case .studyMinutes:
            ...
            : "Study \(dailyStudyGoalMinutes) minutes today."
    ...
    var dailyChallengeProgressText: String {
        switch dailyChallengeType {
        case .studyMinutes:
            return "Today \(todayStudyMinutes)/\(dailyStudyGoalMinutes) min."
        case .completeSession:
    ...
    var remainingDailyGoalText: String {
        switch dailyChallengeType {
        case .studyMinutes:
            let remaining = max(dailyStudyGoalMinutes - todayStudyMinutes, 0)
    ...
    var achievements: [Achievement] {
        [
            Achievement(
                ...
                targetValue: 1,
                progressText: "\(totalGoalCount)/1 goals"
            ),
            Achievement(
                title: "Goal Keeper",
                description: "Create 5 learning goals.",
                icon: "rosette",
                currentValue: totalGoalCount,
                targetValue: 5,
